		...
		<div class="container in_sub_sort">
			<div class="row">
				<div class="col">
					...
					<c:if test="${fn:indexOf( fullURL , '/user/mypage/view') > -1 
					|| fn:indexOf( fullURL , '/user/mypage/update') > -1
					|| fn:indexOf( fullURL , '/user/mypage/answer/list') > -1
					}">
						<ul class="nav gab_03">
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/mypage/view') > -1}">active</c:if> " href="/user/mypage/view.do?idx=${sessionScope.UserIdx }">마이페이지</a></li>
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/mypage/update') > -1}">active</c:if> " href="/user/mypage/update.do?idx=${sessionScope.UserIdx }">회원 수정</a></li>
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/mypage/answer/list') > -1}">active</c:if> " href="/user/mypage/answer/list.do">설문 답변 리스트</a></li>
						</ul>
					</c:if>
					<c:if test="${fn:indexOf( fullURL , '/user/board_data/list.do?board_idx=7') > -1 
					|| fn:indexOf( fullURL , '/user/board_data/list.do?board_idx=8') > -1
					|| fn:indexOf( fullURL , '/user/survey/view.do?idx=1') > -1
					}">
						<ul class="nav gab_03">
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/board_data/list.do?board_idx=7') > -1}">active</c:if> "  href="/user/board_data/list.do?board_idx=7"><span>공지사항</span></a></li>
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/board_data/list.do?board_idx=8') > -1}">active</c:if> " href="/user/board_data/list.do?board_idx=8"><span>정보안내</span></a></li>
							<li><a class="nav-link <c:if test="${fn:indexOf( fullURL , '/user/survey/view.do?idx=1') > -1}">active</c:if> " href="/user/survey/view.do?idx=1"><span>기술이전문의</span></a></li>
						</ul>
					</c:if>
				</div>
			</div>
